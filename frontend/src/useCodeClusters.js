 ...
 * Checked = hide parcels with codes in that group.
 *
...
export function defaultExcludeClusters() {
  return Object.fromEntries(
    USE_CODE_CLUSTERS.map((cluster) => [cluster.id, cluster.defaultExcluded]),
  )
}

export function makeExcludedCodeSet(excludeClusters) {
  const codes = new Set()
  if (!excludeClusters) return codes
  for (const cluster of USE_CODE_CLUSTERS) {
    if (!excludeClusters[cluster.id]) continue
...
export function attachExcludedCodes(filters) {
  ...
  return {
    ...filters,
    _excludedUseCodes: makeExcludedCodeSet(filters.excludeClusters),
  }
}
